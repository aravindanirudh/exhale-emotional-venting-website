    ...
    <div className="card hover:shadow-md transition-shadow duration-200">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${mood.bg} ${mood.color}`}>
                {mood.label}
              </span>
            </div>
      ...
      {post.autoDelete?.enabled && (
        <div className="text-xs text-orange-500 mb-4 flex items-center">
          <Clock className="w-3 h-3 mr-1" />
          Auto-deletes {formatDistanceToNow(new Date(post.autoDelete.deleteAt), { addSuffix: true })}
        </div>
      )}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-dark-border">
        <div className="flex space-x-4">
           ...
           >
             ...
             <span className="text-sm font-medium text-black group-hover:text-white dark:text-black">{post.reactionCounts?.['❤️'] || 0}</span>
           </button>
        </div>
        ...
        >
          ...
          <span className="text-sm font-medium text-black group-hover:text-white dark:text-black">{post.commentCount} Comments</span>
        </Link>
      </div>
    </div>
  );
